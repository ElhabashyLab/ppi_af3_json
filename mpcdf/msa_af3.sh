...
while IFS= read -r dir; do
    ...
    while true; do
        # Count current jobs
        current_jobs=$(squeue -u "$USER_NAME" -p small | tail -n +2 | wc -l)

        if [ "$current_jobs" -lt "$MAX_JOBS" ]; then
            echo "[$(date)] Submitting job in $dir (current jobs: $current_jobs)"
            (
                cd "$dir" || exit
                #jobid=$(sbatch jobscript-alphafold3-step_1-msa.sh | awk '{print $4}')
                sbatch jobscript-alphafold3-step_1-msa.sh
            )
